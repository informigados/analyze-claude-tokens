...
Analyzes Claude JSONL session logs from common local paths such as:
- ~/.claude/projects
- ~/.claude/sessions
...
TRANSLATIONS = {
    "en": {
        "app_description": "Analyze local Claude session logs and generate usage reports.",
        "error_prefix": "Error",
        ...
        "report_title": "# Claude Token Usage Analysis",
        ...
        "unknown": "unknown",
        "console_scanning": "Scanning Claude sessions...",
        "console_found_projects": "Found {count} projects",
        "console_found_files": "Found {count} JSONL files across {dirs} configured directories",
        "console_no_jsonl_hint": "No JSONL session files were found. Verify Claude transcript locations or pass --session-dir.",
    ...
    "pt-br": {
        "app_description": "Analisa logs locais de sessões do Claude e gera relatórios de uso.",
        "error_prefix": "Erro",
        ...
        "report_title": "# Análise de Uso de Tokens do Claude",
        ...
        "console_scanning": "Analisando sessões do Claude...",
        "console_found_projects": "Encontrados {count} projetos",
        "console_found_files": "Encontrados {count} arquivos JSONL em {dirs} diretórios configurados",
        "console_no_jsonl_hint": "Nenhum arquivo JSONL de sessão foi encontrado. Verifique os caminhos de transcript do Claude ou use --session-dir.",
    ...
    "pt-pt": {
        "app_description": "Analisa registos locais de sessões do Claude e gera relatórios de utilização.",
        "error_prefix": "Erro",
        "table_project": "Projeto",
        "table_sessions": "Sessões",
        "table_total_tokens": "Tokens Totais",
        ...
        "date_range_since": "Desde {date}",
        "date_range_all_time": "Todo o período",
        "report_title": "# Análise de Utilização de Tokens do Claude",
        "report_generated": "Gerado em: {now} | Período: {date_range}",
        "report_grand_totals": "## Totais Gerais",
        "report_projects": "- **Projetos**: {count}",
        "report_sessions": "- **Sessões**: {count}",
        ...
        "prompts_session_item": "## {index}. [{timestamp}] Sessão `{session_id}`",
        "unknown": "desconhecido",
        "console_scanning": "A analisar sessões do Claude...",
        "console_found_projects": "Encontrados {count} projetos",
        "console_found_files": "Encontrados {count} ficheiros JSONL em {dirs} diretórios configurados",
        "console_no_jsonl_hint": "Não foram encontrados ficheiros JSONL de sessão. Verifique os caminhos de transcript do Claude ou use --session-dir.",
        "console_total_summary": "Total: {tokens} tokens em {sessions} sessões de {projects} projetos",
    ...
    "es": {
        "app_description": "Analiza registros locales de sesiones de Claude y genera informes de uso.",
        "error_prefix": "Error",
        ...
        "report_title": "# Análisis de Uso de Tokens de Claude",
        ...
        "unknown": "desconocido",
        "console_scanning": "Escaneando sesiones de Claude...",
        "console_found_projects": "Se encontraron {count} proyectos",
        "console_found_files": "Se encontraron {count} archivos JSONL en {dirs} directorios configurados",
        "console_no_jsonl_hint": "No se encontraron archivos JSONL de sesión. Verifica las rutas de transcript de Claude o usa --session-dir.",
...
def dedupe_paths(paths: list[Path]) -> list[Path]:
    ...
    for path in paths:
        ...
        normalized = str(expanded).replace("\\", "/").rstrip("/").lower()
...
def get_default_claude_home() -> Path:
    return Path(
...
def build_default_session_dirs(claude_home: Path) -> list[Path]:
    directories = [
    ...
    if appdata:
        directories.append(Path(appdata) / "Claude" / "local-agent-mode-sessions")
    local_appdata = os.environ.get("LOCALAPPDATA")
    if local_appdata:
        directories.append(
            Path(local_appdata) / "AnthropicClaude" / "local-agent-mode-sessions"
        )
    return dedupe_paths(directories)


def parse_session_dirs_list(raw_value: str | None) -> list[Path]:
    if not raw_value:
        return []
    chunks = [item.strip() for item in str(raw_value).split(os.pathsep)]
    return [Path(chunk).expanduser() for chunk in chunks if chunk]


def build_session_dirs(
    claude_home: Path,
    cli_session_dirs: list[str] | None = None,
) -> list[Path]:
    env_dirs = parse_session_dirs_list(os.environ.get("CLAUDE_SESSION_DIRS"))
    cli_dirs = [Path(item).expanduser() for item in (cli_session_dirs or []) if item]
    return dedupe_paths(build_default_session_dirs(claude_home) + env_dirs + cli_dirs)


CLAUDE_HOME = get_default_claude_home()
SESSION_DIRS = build_session_dirs(CLAUDE_HOME)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="append",
        default=None,
        help="Additional session transcript directory (repeatable).",
    )
    parser.add_argument(
        "--redact-prompts",
...
def configure_runtime(args: argparse.Namespace) -> None:
    ...
    SESSION_DIRS = build_session_dirs(CLAUDE_HOME, args.session_dirs)
...
def derive_project_name(meta: dict[str, Any]) -> str:
    for key in ("project", "project_name", "workspace"):
        value = meta.get(key)
...
def classify_workspace(cwd: str | None) -> str:
    ...
    claude_home_normalized = normalize_path_for_compare(str(CLAUDE_HOME))
    worktree_marker = f"{claude_home_normalized}/worktrees"
...
def normalize_prompt_for_display(text: str) -> str:
    ...
    cleaned = re.sub(r"## My request for [^:]+:", "User request:", cleaned)
...
def map_usage_payload(raw_usage: Any) -> dict[str, int]:
    if not isinstance(raw_usage, dict):
        return empty_usage_totals()
    cached_input_tokens = raw_usage.get(
        "cached_input_tokens",
        raw_usage.get("cache_read_input_tokens", 0),
    )
    reasoning_output_tokens = raw_usage.get(
        "reasoning_output_tokens",
        raw_usage.get("thinking_tokens", raw_usage.get("reasoning_tokens", 0)),
    )
    mapped = {
        "input_tokens": safe_int(raw_usage.get("input_tokens", 0)),
        "cached_input_tokens": safe_int(cached_input_tokens),
        "output_tokens": safe_int(raw_usage.get("output_tokens", 0)),
        "reasoning_output_tokens": safe_int(reasoning_output_tokens),
        "total_tokens": safe_int(raw_usage.get("total_tokens", 0)),
    }
    if mapped["total_tokens"] <= 0:
        mapped["total_tokens"] = (
            mapped["input_tokens"]
            + mapped["cached_input_tokens"]
            + mapped["output_tokens"]
            + mapped["reasoning_output_tokens"]
        )
    return mapped


def usage_has_values(usage: dict[str, int]) -> bool:
    return any(value > 0 for value in usage.values())


def extract_usage_candidates(obj: dict[str, Any]) -> list[dict[str, int]]:
    payload = obj.get("payload")
    info = payload.get("info") if isinstance(payload, dict) else None
    message = obj.get("message")
    candidates: list[Any] = [
        info.get("total_token_usage") if isinstance(info, dict) else None,
        info.get("token_usage") if isinstance(info, dict) else None,
        payload.get("usage") if isinstance(payload, dict) else None,
        payload.get("token_usage") if isinstance(payload, dict) else None,
        payload.get("total_token_usage") if isinstance(payload, dict) else None,
        message.get("usage") if isinstance(message, dict) else None,
        message.get("token_usage") if isinstance(message, dict) else None,
        obj.get("usage"),
        obj.get("token_usage"),
    ...
    for candidate in candidates:
        usage = map_usage_payload(candidate)
        if usage_has_values(usage):
            usage_events.append(usage)
    return usage_events


def choose_usage_totals(usage_events: list[dict[str, int]]) -> dict[str, int]:
    if not usage_events:
        return empty_usage_totals()

    summed = empty_usage_totals()
    for usage in usage_events:
        for key in summed:
            summed[key] += usage.get(key, 0)

    cumulative_totals = [u["total_tokens"] for u in usage_events if u["total_tokens"] > 0]
    if len(cumulative_totals) >= 2 and all(
        prev <= curr for prev, curr in zip(cumulative_totals, cumulative_totals[1:])
    ):
        chosen = usage_events[-1].copy()
    else:
        chosen = summed

    if chosen["total_tokens"] <= 0:
        chosen["total_tokens"] = (
            chosen["input_tokens"]
            + chosen["cached_input_tokens"]
            + chosen["output_tokens"]
            + chosen["reasoning_output_tokens"]
        )
    return chosen
...
def extract_text_from_value(value: Any) -> str:
    ...
    if isinstance(value, dict):
        ...
        if isinstance(text_value, str) and text_value:
            return sanitize_text(text_value)
        if "content" in value:
            extracted_content = extract_text_from_value(value.get("content"))
            if extracted_content:
                return extracted_content
        for key in ("message", "prompt", "input", "output"):
            extracted = extract_text_from_value(value.get(key))
            if extracted:
                return extracted
    return ""


def extract_user_prompt(obj: dict[str, Any]) -> str:
    obj_type = sanitize_text(obj.get("type")).lower()
    role = sanitize_text(obj.get("role")).lower()
    payload = obj.get("payload")
    payload_type = (
        sanitize_text(payload.get("type")).lower() if isinstance(payload, dict) else ""
    )
    payload_role = (
        sanitize_text(payload.get("role")).lower() if isinstance(payload, dict) else ""
    )

    if obj_type == "event_msg" and payload_type == "user_message":
        return extract_text_from_value(payload.get("message")) if isinstance(payload, dict) else ""

    user_markers = {"user", "human", "user_message", "userpromptsubmit"}
    if (
        obj_type in user_markers
        or role in user_markers
        or payload_type in user_markers
        or payload_role in user_markers
    ):
        candidates = [
            obj.get("message"),
            obj.get("content"),
        ...
        for candidate in candidates:
            extracted = extract_text_from_value(candidate)
            if extracted:
                return extracted
    return ""


def extract_assistant_message(obj: dict[str, Any]) -> str:
    obj_type = sanitize_text(obj.get("type")).lower()
    role = sanitize_text(obj.get("role")).lower()
    payload = obj.get("payload")
    payload_type = (
    ...
    if obj_type == "event_msg" and payload_type == "task_complete":
        return (
            extract_text_from_value(payload.get("last_agent_message"))
            if isinstance(payload, dict)
            else ""
        )

    assistant_markers = {"assistant", "model", "assistant_message"}
    if (
        obj_type in assistant_markers
        or role in assistant_markers
        or payload_type in assistant_markers
        or payload_role in assistant_markers
    ):
        candidates = [
            obj.get("message"),
            obj.get("content"),
...
def build_session_result(
    ...
    max_user_instruction_chars: int,
    turn_count: int,
) -> dict[str, Any]:
    base_instructions = meta.get("base_instructions")
    base_instruction_text = ""
    if isinstance(base_instructions, dict):
        base_instruction_text = sanitize_text(base_instructions.get("text"))
    elif isinstance(base_instructions, str):
        base_instruction_text = sanitize_text(base_instructions)
    elif isinstance(meta.get("system_prompt"), str):
        base_instruction_text = sanitize_text(meta.get("system_prompt"))
    base_instruction_chars = len(base_instruction_text)
    instruction_chars = base_instruction_chars + max_user_instruction_chars

    session_id = meta.get("id") or meta.get("session_id") or jsonl_path.stem
    timestamp_start = meta.get("timestamp") or first_obj.get("timestamp")
...
def parse_legacy_style_session(
    jsonl_path: Path,
    objects: list[dict[str, Any]],
) -> dict[str, Any]:
    first = objects[0]
    meta = first.get("payload", {})
    usage_total = empty_usage_totals()
    prompts: list[dict[str, Any]] = []
    final_message = None
    ...
    for obj in objects[1:]:
        ...
        if payload_type == "token_count":
            ...
            if totals:
                usage_total = map_usage_payload(totals)
...
def parse_claude_style_session(
    jsonl_path: Path,
    objects: list[dict[str, Any]],
) -> dict[str, Any]:
    first = objects[0]
    ...
    for obj in objects:
        payload = obj.get("payload")
        if isinstance(payload, dict):
            if obj.get("type") in {"session_meta", "session_start"} and not meta:
                meta.update(payload)
            for key in (
                "id",
                "session_id",
        ...
        if isinstance(payload, dict):
            user_instructions = payload.get("user_instructions") or payload.get("instructions")
        if not isinstance(user_instructions, str):
            user_instructions = obj.get("user_instructions")
        if isinstance(user_instructions, str):
            max_user_instruction_chars = max(max_user_instruction_chars, len(user_instructions))

        usage_events.extend(extract_usage_candidates(obj))

        prompt_text = extract_user_prompt(obj)
        if prompt_text:
            turn_count += 1
            timestamp = sanitize_text(obj.get("timestamp"))
            dedupe_key = (timestamp, prompt_text)
            if dedupe_key not in seen_prompt_keys:
                prompts.append({"text": prompt_text, "timestamp": obj.get("timestamp")})
                seen_prompt_keys.add(dedupe_key)

        assistant_message = extract_assistant_message(obj)
        if assistant_message:
            final_message = assistant_message

    usage_total = choose_usage_totals(usage_events)
...
def parse_session(jsonl_path: Path) -> dict[str, Any] | None:
    ...
    if first.get("type") == "session_meta":
        return parse_legacy_style_session(jsonl_path, objects)
    return parse_claude_style_session(jsonl_path, objects)


def analyze_all(session_files: list[Path] | None = None) -> tuple[
    ...
    files_to_scan = session_files if session_files is not None else iter_session_files()
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(tr("console_found_files", count=len(session_files), dirs=len(SESSION_DIRS)))
    if not session_files:
        print(tr("console_no_jsonl_hint"))
        for directory in SESSION_DIRS:
            print(f"  - {directory}")
    projects, children_by_parent, sessions_by_id = analyze_all(session_files)
