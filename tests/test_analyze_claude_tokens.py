...
SCRIPT_PATH = Path(__file__).resolve().parents[1] / "analyze-claude-tokens.py"
TEST_TMP_ROOT = Path(__file__).resolve().parents[1] / ".tmp-test"
TEST_TMP_ROOT.mkdir(parents=True, exist_ok=True)


def build_tmp_file(name: str) -> Path:
    run_dir = TEST_TMP_ROOT / f"run-{uuid.uuid4().hex}"
    run_dir.mkdir(parents=True, exist_ok=True)
    return run_dir / name


def load_module():
    spec = importlib.util.spec_from_file_location("analyze_claude_tokens", SCRIPT_PATH)
...
class AnalyzeClaudeTokensTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.mod = load_module()

    def test_parse_session_extracts_usage_and_prompt(self):
        file_path = build_tmp_file("tmp-test-session.jsonl")
        lines = [
            {
                "type": "session_meta",
    ...
    def test_normalize_prompt_for_display(self):
        normalized = self.mod.normalize_prompt_for_display(
            ...
            "## My request for Claude: Explain this function\n"
    ...
    def test_parse_session_dirs_list_parses_multiple_paths(self):
        raw = os.pathsep.join(["C:/logs/a", "C:/logs/b"])
        parsed = self.mod.parse_session_dirs_list(raw)
        self.assertEqual(len(parsed), 2)
        self.assertTrue(str(parsed[0]).replace("\\", "/").lower().endswith("c:/logs/a"))
        self.assertTrue(str(parsed[1]).replace("\\", "/").lower().endswith("c:/logs/b"))

    def test_build_session_dirs_merges_default_env_and_cli(self):
        cli_dir = Path("C:/cli-sessions")
        env_dir = Path("C:/env-sessions")
        with patch.dict(
            os.environ,
            {"CLAUDE_SESSION_DIRS": str(env_dir)},
            clear=False,
        ):
            session_dirs = self.mod.build_session_dirs(
                Path("C:/claude-home"),
                [str(cli_dir)],
            )

        normalized = {str(item).replace("\\", "/").lower() for item in session_dirs}
        self.assertIn(str(cli_dir).replace("\\", "/").lower(), normalized)
        self.assertIn(str(env_dir).replace("\\", "/").lower(), normalized)
    ...
    def test_parse_session_supports_claude_style_transcript(self):
        file_path = build_tmp_file("tmp-test-claude-session.jsonl")
        lines = [
            {
                "type": "session_start",
            ...
            },
            {
                ...
                "message": {
                    "content": [
                        {"type": "text", "text": "hello from claude transcript"}
                    ]
                },
            },
            {
        ...
        session = self.mod.parse_session(file_path)
        self.assertIsNotNone(session)
        self.assertEqual(session["session_id"], "claude-sess-1")
        self.assertEqual(session["project"], "claude-project")
        self.assertEqual(session["usage"]["input_tokens"], 120)
        self.assertEqual(session["usage"]["cached_input_tokens"], 30)
        self.assertEqual(session["usage"]["output_tokens"], 40)
        self.assertEqual(session["usage"]["reasoning_output_tokens"], 10)
        self.assertEqual(session["usage"]["total_tokens"], 200)
        self.assertEqual(session["prompt_count"], 1)
        self.assertEqual(session["prompts"][0]["text"], "hello from claude transcript")
